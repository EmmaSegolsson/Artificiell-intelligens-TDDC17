...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while( !frontier.isEmpty() ) {
			//Look at next in queue and remove.
			SearchNode myNode = frontier.peekAtFront(); 
			frontier.removeFirst();
			
			if( p.isGoalState(myNode.getState()) ) {
				path = myNode.getPathFromRoot();
				return path;
			}
			
			//Expand children.
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(myNode.getState());
			
			for( int i = 0; i < childStates.size(); i++) {
				GridPos s = childStates.get(i);

				if(!explored.contains(new SearchNode(s)) && !frontier.contains(new SearchNode(s))) {
					if(insertFront) {
						frontier.addNodeToFront(new SearchNode(s, myNode));
					}else {
						frontier.addNodeToBack(new SearchNode(s, myNode));
					}
					explored.add(new SearchNode(s, myNode));
				}
			}
		}
